(function() {
  var Nav;

  Nav = {
    init: function() {
      this.setup();
      return this.uiBind();
    },
    setup: function() {
      return $('.mainnav').find('li').toggleClass('invisible');
    },
    uiBind: function() {
      return $(document).on('click', '.mainnav', function(e) {
        e.preventDefault();
        return $(this).find('li').toggleClass('animate').toggleClass('invisible');
      });
    }
  };

  Nav.init();

}).call(this);

$(document).on('mouseleave', 'mainnav', function () {
       $(this).find('.mainnav-ul').slideUp('fast');
});



//$(".mainnav-b").click(function() {
    
//}),



//# sourceMappingURL=data:application/json;base64,
//# sourceURL=coffeescript